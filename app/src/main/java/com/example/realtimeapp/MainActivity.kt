...
class MainActivity : AppCompatActivity(), ImageReader.OnImageAvailableListener {
    ...
    protected fun setFragment() {
        ...
        // Find a back-facing camera ID (don’t assume index 0/1)
        val cameraId: String? = try {
            manager.cameraIdList.firstOrNull { id ->
                val chars = manager.getCameraCharacteristics(id)
                chars.get(CameraCharacteristics.LENS_FACING) == CameraCharacteristics.LENS_FACING_BACK
            } ?: manager.cameraIdList.firstOrNull() // fallback to the first available camera
        } catch (e: Exception) {
            e.printStackTrace()
            null
        }
        ...
        if (cameraId == null) {
            // No camera available or access error—bail out gracefully
